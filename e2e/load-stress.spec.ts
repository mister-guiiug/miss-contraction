...
test.describe('Stress - Actions rapides', () => {
  ...
  test('@stress clics rapides multiples sur bouton ne causent pas d’état incohérent', async ({
    page,
  }) => {
    const btn = page.locator('[data-testid="toggle-contraction-btn"]');
    await expect(btn).toBeVisible();
  ...
  test('@stress navigation rapide entre vues avec données', async ({
    page,
  }) => {
    // Charger des données
    await page.evaluate(
...
test.describe('Stress - Formulaires', () => {
  test('@stress sauvegarder les paramètres 5 fois de suite', async ({
    page,
  }) => {
    ...
    for (let i = 0; i < 5; i++) {
      await page
        .locator('[data-testid="max-interval-input"]')
        .fill(String(5 + i));
      await saveBtn.click();
      await page.waitForTimeout(200);
    }
...
test.describe('Charge - Messages et message view', () => {
  test('@load message très long (1000 chars) dans textarea', async ({
    page,
  }) => {
  ...
  test('@load effacer et réécrire le message plusieurs fois', async ({
    page,
  }) => {
    await page.goto(ROUTES.MESSAGE);
    await page.waitForLoadState('networkidle');
