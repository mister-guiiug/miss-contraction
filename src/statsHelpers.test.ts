...
describe("filterRecordsByStatsWindow", () => {
  ...
  it('retourne tout quand key="all"', () => {
    expect(filterRecordsByStatsWindow([r1, r2, r3], "all", now)).toHaveLength(3);
  });
...
describe("getRecentIntervalsMs", () => {
  ...
  it("limite au nombre max demandé", () => {
    const records = Array.from({ length: 10 }, (_, i) => makeRecord(i * 60_000, 30_000));
...
describe("computeThresholdBadge", () => {
  ...
  it('retourne "calm" si une seule contraction terminée', () => {
    const now = Date.now();
    expect(
      computeThresholdBadge([makeRecord(now - 5 * 60_000, 60_000)], defaultSettings)
    ).toBe("calm");
  });
});
